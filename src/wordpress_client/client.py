...
class WordPressClient:
    ...
    def _request(
        ...
        try:
            ...
            if response.status_code == 401:
                ...
                raise AuthenticationError(error_msg, response.status_code, response.json())
                ...
                error_msg = parse_wp_error(response.json() if response.text else {})
                raise WordPressAPIError(error_msg, response.status_code, response.json())
    ...
    def update_category(self, category_id: int, category: CategoryUpdate) -> Category:
        ...
        """
        data = category.model_dump(exclude_none=True)
        response_data = self._request("POST", f"wp/v2/categories/{category_id}", data=data)
        return Category(**response_data)
    ...
    def delete_category(self, category_id: int, force: bool = False) -> Dict:
        ...
        """
        params = {"force": force}
        return self._request("DELETE", f"wp/v2/categories/{category_id}", params=params)
    ...
    def upload_media(self, file_path: str, title: Optional[str] = None, 
                     ...
                     post_id: Optional[int] = None) -> Media:
        """
        Upload a media file.
    ...
    def delete_media(self, media_id: int, force: bool = False) -> Dict:
        ...
        """
        params = {"force": force}
        return self._request("DELETE", f"wp/v2/media/{media_id}", params=params)
