...
def get_client() -> WordPressClient:
    ...
    if not all([url, username, password]):
        click.echo(
            "Error: Missing WordPress credentials in environment variables.", err=True
        )
        click.echo("Required: WP_URL, WP_USERNAME, WP_PASSWORD", err=True)
        sys.exit(1)

    return WordPressClient(base_url=url, username=username, password=password)
...
def create_post(
    ...
    try:
        ...
        cat_ids = (
            [int(c.strip()) for c in categories.split(",")] if categories else None
        )
...
def get_post(post_id: int):
    ...
    try:
        ...
        title = post.title.rendered if post.title else "No Title"
        content = "No content"
        if post.content and post.content.rendered:
...
def update_post(
    post_id: int, title: Optional[str], content: Optional[str], status: Optional[str]
):
    """Update an existing post."""
    try:
        client = get_client()

        update_data = PostUpdate(title=title, content=content, status=status)
...
def delete_post(post_id: int, force: bool):
    ...
    try:
        client = get_client()
        _ = client.delete_post(post_id, force=force)
...
def create_category(
    name: str, description: Optional[str], slug: Optional[str], parent: Optional[int]
):
    ...
    try:
        ...
        cat_data = CategoryCreate(
            name=name, description=description, slug=slug, parent=parent or 0
        )

        category = client.create_category(cat_data)
        click.echo("✓ Category created successfully!")
...
def delete_category(category_id: int, force: bool):
    ...
    try:
        client = get_client()
        _ = client.delete_category(category_id, force=force)
...
def upload_media(
    ...
    try:
        ...
        click.echo("✓ Media uploaded successfully!")
...
def list_media(
    page: int, per_page: int, search: Optional[str], media_type: Optional[str]
):
    """List media items."""
    try:
        client = get_client()
        items = client.get_media(
            page=page, per_page=per_page, search=search, media_type=media_type
        )
...
def delete_media(media_id: int, force: bool):
    ...
    try:
        client = get_client()
        _ = client.delete_media(media_id, force=force)
